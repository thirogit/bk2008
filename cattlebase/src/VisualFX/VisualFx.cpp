...
void TTabWnd::ResizeTab(int cx, int cy)
{
  ...
  if (pParent->IsKindOf(RUNTIME_CLASS(CSplitterWnd))) {
    ...
    ASSERT(pParent);
    int row,col;
    splitter->IsChildPane(this,&row,&col); //THIRO orig. splitter->IsChildPane(this,row,col);
...
{
  ASSERT(pMsg);
  if (pMsg->message == WM_KEYDOWN || (pMsg->message == WM_SYSKEYDOWN && pMsg->wParam == VK_F10)) 
  {
	  return HandleKeyDown(pMsg->wParam,pMsg->hwnd);
  }
  return FALSE;
}

BOOL TVisualFramework::HandleKeyDown(UINT key,HWND hwnd)
{
    // Handle Ctrl+Tab for tab windows
    if (m_bEnableCtrlTab) 
	{
      if ((key == VK_TAB) && (::GetAsyncKeyState(VK_CONTROL) != 0)) 
	  {
