...
CDocProperty::CDocProperty( DWORD dwID, const LPVOID pValue, DWORD dwType )
{
	m_dwPropID = dwID;
	m_dwType = dwType;
	m_pValue = NULL;       // must init to NULL
...
BOOL CDocProperty::Set( DWORD dwID, const LPVOID pValue, DWORD dwType )
{
	m_dwType = dwType;
	m_dwPropID = dwID;
...
BOOL CDocProperty::Set( const LPVOID pValue, DWORD dwType )
{
	m_dwType = dwType;
	return Set( pValue );
}

BOOL CDocProperty::Set( const  LPVOID pVal )
{
	ULONG           cb;
	ULONG           cbItem;
	ULONG           cbValue;
	...
	{
		TRACE0("CDocProperty::AllocValue failed");
		return FALSE;
	}
...
LPVOID CDocProperty::Get( void )
{   return Get( (DWORD*)NULL );   }

LPVOID CDocProperty::Get( DWORD* pcb )
{
	DWORD   cb = 0;
	LPBYTE  p = NULL;
...
DWORD  CDocProperty::GetType( void )
{   return m_dwType;  }

void   CDocProperty::SetType( DWORD dwType )
{   m_dwType = dwType; }

DWORD CDocProperty::GetID( void )
{   return m_dwPropID;   }

void CDocProperty::SetID( DWORD dwPropID )
{    m_dwPropID = dwPropID;   }

LPVOID CDocProperty::GetRawValue( void )
{   return m_pValue; }

BOOL CDocProperty::WriteToStream( IStream* pIStream )
{
	ULONG           cb = 0;
...
BOOL CDocProperty::ReadFromStream( IStream* pIStream )
{
	ULONG           cb;
	ULONG           cbItem;
	ULONG           cbValue;
...
LPVOID CDocProperty::AllocValue(ULONG cb)
{
	return m_pValue = malloc((int)cb);
}


void CDocProperty::FreeValue()
{
	if (m_pValue != NULL)
	{
		free(m_pValue);
...
/////////////////////////////////////////////////////////////////////////////
// Implementation of the CDocPropertySection Class

CDocPropertySection::CDocPropertySection( void )
{
...
CDocPropertySection::CDocPropertySection( CLSID FormatID )
{
	m_FormatID = FormatID;
	m_SH.cbSection = 0;
	m_SH.cProperties = 0;
...
CLSID CDocPropertySection::GetFormatID( void )
{   return m_FormatID; }

void CDocPropertySection::SetFormatID( CLSID FormatID )
{   m_FormatID = FormatID; }

BOOL CDocPropertySection::Set( DWORD dwPropID, LPVOID pValue, DWORD dwType )
{
	CDocProperty* pProp = GetProperty( dwPropID );
	if (pProp == NULL)
	{
		if ((pProp = new CDocProperty( dwPropID, pValue, dwType )) != NULL)
			AddProperty( pProp );
		return (pProp != NULL);
	}
...
BOOL CDocPropertySection::Set( DWORD dwPropID, LPVOID pValue )
{
...
LPVOID CDocPropertySection::Get( DWORD dwPropID )
{   return Get( dwPropID, (DWORD*)NULL );  }

LPVOID CDocPropertySection::Get( DWORD dwPropID, DWORD* pcb )
{
	CDocProperty* pProp = GetProperty( dwPropID );
	if (pProp)
		{
...
void CDocPropertySection::Remove( DWORD dwID )
{
	...
	while( pos != NULL )
	{
		...
		pProp = (CDocProperty*)m_PropList.GetNext( pos );
...
void CDocPropertySection::RemoveAll( )
{
	POSITION pos = m_PropList.GetHeadPosition();
	while( pos != NULL )
		delete (CDocProperty*)m_PropList.GetNext( pos );
	m_PropList.RemoveAll();
	m_SH.cProperties = 0;
}


CDocProperty* CDocPropertySection::GetProperty( DWORD dwPropID )
{
	POSITION pos = m_PropList.GetHeadPosition();
#if (defined _DEBUG)
...
void CDocPropertySection::AddProperty( CDocProperty* pProp )
{
	m_PropList.AddTail( pProp );
	m_SH.cProperties++;
}

DWORD CDocPropertySection::GetSize( void )
{   return m_SH.cbSection; }

DWORD CDocPropertySection::GetCount( void )
{   return m_PropList.GetCount();  }

CObList* CDocPropertySection::GetList( void )
{   return &m_PropList;  }

BOOL CDocPropertySection::WriteToStream( IStream* pIStream )
{
...
BOOL CDocPropertySection::ReadFromStream( IStream* pIStream,
	LARGE_INTEGER liPropSet )
{
	...
	while (cProperties--)
	{
		...
		{
			pProp = new CDocProperty( pido.propertyID, NULL, 0 );
...
BOOL CDocPropertySection::GetID( LPCTSTR pszName, DWORD* pdwPropID )
{
...
BOOL CDocPropertySection::SetName( DWORD dwPropID, LPCTSTR pszName )
{
	BOOL bSuccess = TRUE;
	CString strName(pszName);
	strName.MakeLower();        // Dictionary stores all names in lowercase
...
BOOL CDocPropertySection::ReadNameDictFromStream( IStream* pIStream )
{
	ULONG cb;
	ULONG cbRead = 0;
...
BOOL CDocPropertySection::WriteNameDictToStream( IStream* pIStream )
{
	ULONG cb;
...
BOOL CDocPropertySection::SetSectionName( LPCTSTR pszName )
{
	m_strSectionName = pszName;
	return TRUE;
}

LPCTSTR CDocPropertySection::GetSectionName( void )
{
	return (LPCTSTR)m_strSectionName;
}

CDocPropertySet::CDocPropertySet( void )
{
...
CDocPropertySet::CDocPropertySet( CLSID clsID )
{
	m_PH.wByteOrder = 0xFFFE;
	m_PH.wFormat = 0;
	m_PH.dwOSVer = (DWORD)MAKELONG( LOWORD(GetVersion()), 2 );
...
CDocPropertySet::~CDocPropertySet()
{   RemoveAll();  }

BOOL CDocPropertySet::Set( CLSID FormatID, DWORD dwPropID, LPVOID pValue, DWORD dwType )
{
	CDocPropertySection* pSect = GetSection( FormatID );
	if (pSect == NULL)
	{
		if ((pSect = new CDocPropertySection( FormatID )) != NULL)
			AddSection( pSect );
	}
...
BOOL CDocPropertySet::Set( CLSID FormatID, DWORD dwPropID, LPVOID pValue )
{
...
LPVOID CDocPropertySet::Get( CLSID FormatID, DWORD dwPropID, DWORD* pcb )
{
	CDocPropertySection* pSect = GetSection( FormatID );
	if (pSect)
...
LPVOID CDocPropertySet::Get( CLSID FormatID, DWORD dwPropID )
{   return Get( FormatID, dwPropID, (DWORD*)NULL ); }

void CDocPropertySet::Remove( CLSID FormatID, DWORD dwPropID )
{
	CDocPropertySection*  pSect = GetSection( FormatID );
	if (pSect)
...
void CDocPropertySet::Remove( CLSID FormatID )
{
	CDocPropertySection* pSect;
	POSITION posRemove = m_SectionList.GetHeadPosition();
	POSITION pos = posRemove;
	while( posRemove != NULL )
	{
		pSect = (CDocPropertySection*)m_SectionList.GetNext( pos );
...
void CDocPropertySet::RemoveAll( )
{
	POSITION pos = m_SectionList.GetHeadPosition();
	while( pos != NULL )
	{
		delete (CDocPropertySection*)m_SectionList.GetNext( pos );
	}
	m_SectionList.RemoveAll();
	m_PH.cSections = 0;
}

CDocPropertySection* CDocPropertySet::GetSection( CLSID FormatID )
{
	POSITION pos = m_SectionList.GetHeadPosition();
	CDocPropertySection* pSect;
	while (pos != NULL)
...
CDocPropertySection* CDocPropertySet::AddSection( CLSID FormatID )
{
	CDocPropertySection* pSect = GetSection( FormatID );
	if (pSect)
		return pSect;

	pSect = new CDocPropertySection( FormatID ) ;
	if (pSect)
		AddSection( pSect );
	return pSect;
}

void CDocPropertySet::AddSection( CDocPropertySection* pSect )
{
	m_SectionList.AddTail( pSect );
	m_PH.cSections++;
}

CDocProperty* CDocPropertySet::GetProperty( CLSID FormatID, DWORD dwPropID )
{
	CDocPropertySection* pSect = GetSection( FormatID );
	if (pSect)
...
void CDocPropertySet::AddProperty( CLSID FormatID, CDocProperty* pProp )
{
	CDocPropertySection* pSect = GetSection( FormatID );
	if (pSect)
...
WORD CDocPropertySet::GetByteOrder( void )
{   return m_PH.wByteOrder;  }

WORD CDocPropertySet::GetFormatVersion( void )
{   return m_PH.wFormat;  }

void CDocPropertySet::SetFormatVersion( WORD wFmtVersion )
{   m_PH.wFormat = wFmtVersion;  }

DWORD CDocPropertySet::GetOSVersion( void )
{   return m_PH.dwOSVer;  }

void CDocPropertySet::SetOSVersion( DWORD dwOSVer )
{   m_PH.dwOSVer = dwOSVer;  }

CLSID CDocPropertySet::GetClassID( void )
{   return m_PH.clsID;  }

void CDocPropertySet::SetClassID( CLSID clsID )
{   m_PH.clsID = clsID;  }

DWORD CDocPropertySet::GetCount( void )
{   return m_SectionList.GetCount();  }

CObList* CDocPropertySet::GetList( void )
{   return &m_SectionList;  }


BOOL CDocPropertySet::WriteToStream( IStream* pIStream )
{
	LPSTREAM        pIStrFIDO;
	FORMATIDOFFSET  fido;
	ULONG           cb;
	...
	// Write each section.
	CDocPropertySection*   pSect = NULL;
	POSITION            pos = m_SectionList.GetHeadPosition();
	while( pos != NULL )
	{
		// Get next element (note cast)
...
BOOL CDocPropertySet::ReadFromStream( IStream* pIStream )
{
	ULONG               cb;
	FORMATIDOFFSET      fido;
	ULONG               cSections;
