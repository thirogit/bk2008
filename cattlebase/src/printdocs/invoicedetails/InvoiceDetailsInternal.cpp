#include "stdafx.h"
#include "InvoiceDetailsInternal.h"
...
IMPLEMENT_DYNCREATE(CInvoiceDetailsInternalDoc, CInvoiceDetailsDoc)

CInvoiceDetailsInternalDoc::CInvoiceDetailsInternalDoc()
{
     
}
void CInvoiceDetailsInternalDoc::PrintBottom()
{
	Money vat,totalGros;
	...
	DrawText("TOTAL_PRIMARY_WEIGHT",CStringFormat("%s kg",(LPCSTR)m_totalWeight.ToString(GetWeightFormat())));
	DrawText("TOTAL_SECONDARY_WEIGHT",CStringFormat("%s kg",(LPCSTR)m_totalSecWeight.ToString(GetWeightFormat())));
...
void CInvoiceDetailsInternalDoc::PrintRecord(int row,int iLP,CowAccessor* pAccessor,Cow *pCow)
{
		DrawText(CStringFormat("LP%d",row),CIntString(iLP));
		DrawText(CStringFormat("FIRSTSTOCK%d",row),(LPCSTR)pCow->GetStock()->GetStockCode());
		DrawText(CStringFormat("SECONDSTOCK%d",row),(LPCSTR)pAccessor->GetStock(pCow)->GetStockCode());
		DrawWeight(CStringFormat("FIRSTWEIGHT%d",row),pCow->GetWeight());
