...
{
	TRACE("in default CXColourPopup::CXColourPopup 1\n");
...
{
	TRACE("in CXColourPopup::CXColourPopup 2\n");
	...
	TRACE("   m_pParent=0x%0X\n", m_pParent);
	TRACE("   setting initial color %08X\n", m_crInitialColor);
...
void CXColourPopup::Init()
{
	...
	m_bShowCustom   = TRUE;
	m_strCustomText.LoadString(IDS_CUSTOMCOLORS);
	m_pParent       = NULL;
...
{
	TRACE(_T("in CXColourPopup::OnLButtonUp\n"));
...
{
	TRACE("in CXColourPopup::ChangeSelection\n");
...
{
	TRACE("in CXColourPopup::EndSelection - m_nCurrentRow=%d\n", m_nCurrentRow);
	...
	else if (nMessage != CPN_SELENDCANCEL && m_nCurrentRow == CUSTOM_BUTTON_ROW)
	{
		...
		COLORREF rgb[CUSTOM_COLORS_COUNT];
		int i, row, col;
