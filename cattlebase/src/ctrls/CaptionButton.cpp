// CaptionButton.cpp: Implementierung der Klasse CCaptionBndutton.
//
//////////////////////////////////////////////////////////////////////
/*! 
...
BOOL CCaptionButton::ResetButtonState()
{
	BOOL bRet = m_bBtnChecked;
	m_bBtnChecked = FALSE;
	return bRet;
}
...
m_rc.left = m_rc.right - sizeButton.cx+2;
	...
	CBitmap *paintBitmap = 	m_bBtnChecked ? &m_bmpBitmapDown : &m_bmpBitmapUp;
	paintBitmap->GetBitmap(&bm);
	...
	// make the new dcMemo compatible to the existing one(dc):
		...
		CBitmap* pOldBitmap = dcMemo.SelectObject(paintBitmap);
...
m_bCheckBtn = bCheckBtn;
	...
	//get button images
	if(m_bmpBitmapUp.m_hObject == NULL)
		VERIFY(m_bmpBitmapUp.LoadBitmap(MAKEINTRESOURCE(m_idBmpUp)));
	if(m_bmpBitmapDown.m_hObject == NULL)
		VERIFY(m_bmpBitmapDown.LoadBitmap(MAKEINTRESOURCE(m_idBmpDown)));
