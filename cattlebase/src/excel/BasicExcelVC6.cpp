	...
	for (size_t s=0; s<maxWorksheets; ++s)
	{
		...
		for (size_t r=0, curRowBlock=0; r<maxRows; ++r)
		{
			...
			for (size_t c=0; c<maxCols; ++c)
			{
				...
				BasicExcelCell::CellTypeEnum cellType = cell->Type();
				if (cellType != BasicExcelCell::TYPE_UNDEFINED)	// Current cell contains some data
				{		
					...
					switch(cellType)
					{
						case BasicExcelCell::TYPE_INT:
						{
							...
							for (; cl<maxCols; ++cl)
							{
								...
								if (cellNext->Type()==BasicExcelCell::TYPE_UNDEFINED ||
						...
						case BasicExcelCell::TYPE_DOUBLE:
						{
							// Check whether it is a single cell or range of cells.
						...
						case BasicExcelCell::TYPE_STRING:
						{
						...
						case BasicExcelCell::TYPE_WSTRING:
						{
							// Fill cell information
							pCell->type_ = CODE::LABELSST;
							pCell->normalType_ = true;
...
void BasicExcelWorksheet::Print(ostream& os, char delimiter, char textQualifier)
{
	for (size_t r=0; r<maxRows_; ++r)
	{
		for (size_t c=0; c<maxCols_; ++c)
		{
			...
			switch (cell->Type())
			{
				case BasicExcelCell::TYPE_UNDEFINED:
					break;
...
/************************************************************************************************************/
BasicExcelCell::BasicExcelCell() : type_(TYPE_UNDEFINED) {};

// Get type of value stored in current Excel cell. 
// Returns one of the enums.
BasicExcelCell::CellTypeEnum BasicExcelCell::Type() const {return type_;}
...
{
	if (type_ == TYPE_STRING) return str_.size() - 1;
