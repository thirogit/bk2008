...
CString CSpreadSheet::ConvertKeyword(CString text)
{
	CString text2="";
	CString text3="";
	if(text.GetLength()==0)
...
// Add header row to spreadsheet
/*bool CSpreadSheet::AddFormats(CStringArray &FieldFormats, bool replace)
{
	if (m_bAppend) // Append to old Sheet
	{
		m_aFormats.Append(FieldFormats);	
	}
	else // New Sheet
	{
		m_aFormats.Copy(FieldFormats);
	}
	return true;
}
*/
...
bool CSpreadSheet::AddRow(CStringArray &RowValues, long row, bool replace)
{
	...
	for (int i = 0; i < RowValues.GetSize(); i++)
	{
		RowValues.SetAt(i,ConvertKeyword(RowValues.GetAt(i)));
...
bool CSpreadSheet::Commit()
{
	if (m_bExcel) // If file is an Excel spreadsheet
	{
		...
		{
			//m_stempSql = m_stempSql + "[" + m_aFieldNames.GetAt(j) +"]" + " " + m_aFormats.GetAt(j) +", ";
...
//			m_stempSql.Format("INSERT INTO [%s$A1:IV%d] (", m_sSheetName, k);
			...
			for (int j = 0; j < m_atempArray.GetSize(); j++)
			{
				//if(!strncmp((LPCTSTR)(m_aFormats.GetAt(j)),"char",4) 
				// ||!strncmp((LPCTSTR)(m_aFormats.GetAt(j)),"TEXT",4)
				// ||!strncmp((LPCTSTR)(m_aFormats.GetAt(j)),"varchar",7))
...
bool CSpreadSheet::Open()
{
	if (m_bExcel) // If file is an Excel spreadsheet
	{
		...
		m_sSql.Format("SELECT * FROM [%s]", m_sSheetName);
		try
		{
