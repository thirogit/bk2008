/*Added a function AddFormats which provides the option to choose different formats for 
 different data. Basically, Excel allows two formats TEXT and NUMBER, so you can now use 
 AddFormats in the same way as AddColumns to specify the format of each column.
 Namely, use something like AddFormats("NUMBER"); or AddFormats("TEXT"); for each column.

 Also corrected several bugs: 
 1. The table specification like "Create Table [Sheet1$A1:IV65535]" doesn't work in 
    my Excel 2000, I modified to "Create Table [Sheet 1]" and works fine.
 2. The memory leak reported by users is because the author didn't deleted the exception 
    pointer when he captured it, I corrected this so there should not be any memory leak now.
 3. If you insert string with single quotation mark, the original program would crash since 
    it didn't handle it. I handled this one in ConvertKeyword so single quotation mark should
    not be a problem now.

 AddFormats might not be very robust but should work fine if you use it right(insert n columns, insert n formats).
*/
//My code: 
...
public:
	...
	//bool AddFormats(CStringArray &FieldFormats, bool replace = false); // Add header row to spreadsheet
...
private:
	...
	CString ConvertKeyword(CString text); // Convert the keywords in text into SQL-compliant manner
	...
	//CStringArray m_aFormats; // Format of the columns
