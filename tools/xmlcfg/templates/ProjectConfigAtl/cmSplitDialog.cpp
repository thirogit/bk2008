...
BOOL cmSplitDialog::OnInitDialog() 
{
	...
	if (embedded_) {
		cmdOk_.ShowWindow(SW_HIDE);
		cmdCancel_.ShowWindow(SW_HIDE);
	}
...
void cmSplitDialog::OnMouseMove( UINT flags, CPoint pt)
{
	if (draggingSplitter_ && (flags & MK_LBUTTON)) {
		CRect rc;
		int minPos;
		int maxPos;
		CPoint point(pt);
...
void cmSplitDialog::OnLButtonDown( UINT nFlags, CPoint pt)
{
	if (HitSplitter(pt)) {
		draggingSplitter_ = true;
		SetCapture();
...
void cmSplitDialog::OnLButtonUp( UINT nFlags, CPoint point )
{
	if (draggingSplitter_) {
		draggingSplitter_ = false;
		ReleaseCapture();
