										 ...
										 IXMLDOMDocument2* xmlDoc)
{
	...
	cmString tmp(W2T(nodeValue));
	AppendMemberNode(nodeName, tmp, parentElement, xmlDoc);
}*/
											...
											BSTR& member)
{
	...
	if (childNode) {
		...
		member = T2BSTR((const TCHAR*)nodeValueStr);	
		childNode->Release();
											   ...
											   cmString& nodeValueStr)
{
	...
	for (long i = 0; i < size; i++) {
		...
		if (nodeName.CompareNoCase(W2T(tmpNodeName)) == 0) {
			...
			nodeValueStr = tmpNodeValue.bstrVal;
			break;
		}
		childNode->Release();
	}
							...
							IXMLDOMDocument2* xmlDoc)
{
	...
	AppendAttributeNode(nodeName, tmp, parentElement, xmlDoc);
}*/
...
void CXmlSerializable::AppendAttributeNode(const cmString& nodeName, bool nodeValue, IXMLDOMElement* parentElement, IXMLDOMDocument2* xmlDoc)
{
	AppendAttributeNode(nodeName, nodeValue ? 1L : 0L, parentElement, xmlDoc);
}

void CXmlSerializable::AppendAttributeNode(const cmString& nodeName, VARIANT_BOOL nodeValue, IXMLDOMElement* parentElement, IXMLDOMDocument2* xmlDoc)
{
...
bool CXmlSerializable::ExtractAttributeNode(IXMLDOMNode* parentElement, const cmString& nodeName, VARIANT_BOOL& member)
{
	cmString tmp;	
	bool ret;
...
/*bool CXmlSerializable::ExtractTextNode(IXMLDOMNode* parentElement, BSTR& member)
{
	USES_CONVERSION;
