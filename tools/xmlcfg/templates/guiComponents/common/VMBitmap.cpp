...
CVMBitmap::CVMBitmap()
{
    ...
    m_nHeight = 0;
    m_hMask = NULL;
	m_clrTransparent = RGB(0xFF, 0, 0xFF);
...
BOOL CVMBitmap::DrawSized(HDC hDC, int left, int top, int width,int height)
{
...
{
	return DrawTiles(hDC, 0, 0, nWidth, nHeight);
}

BOOL CVMBitmap::DrawTiles(HDC hDC, int left, int top, int nWidth, int nHeight)
{
	BOOL bValid = m_nWidth > 0 && m_nHeight > 0;
    ...
    if (bValid == TRUE) {
		...
		for(int y = top; y < top + nHeight; y += m_nHeight) {
			height = min(top + nHeight - y, m_nHeight);
			for(int x = left; x < left + nWidth; x += m_nWidth) {
				width = min(left + nWidth - x, m_nWidth);
				DrawSized(hDC, x, y, width, height);
