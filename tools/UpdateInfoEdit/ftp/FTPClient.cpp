...
bool CFTPClient::Download(const tstring& strRemoteFile, ITransferNotification* pOutput, bool fPasv) const
{
   ...
   for( TObserverSet::const_iterator it=m_setObserver.begin(); it!=m_setObserver.end(); it++ )
      (*it)->OnPreReceiveFile(strRemoteFile, "BUFFER" , lRemoteFileSize);

   const bool fRet=ExecuteDatachannelCommand(CDatachannelCmd::RETR(), 
											strRemoteFile, CRepresentation(nsFTP::CType::Image()),
											fPasv, 0, pOutput);

   for( TObserverSet::const_iterator it2=m_setObserver.begin(); it2!=m_setObserver.end(); it2++ )
      (*it2)->OnPostReceiveFile(strRemoteFile, "BUFFER", lRemoteFileSize);
...
bool CFTPClient::Upload(ITransferNotification* pInput, const tstring& strRemoteFile, bool fPasv) const
{
   CDatachannelCmd cmd(CDatachannelCmd::STOR());

   TObserverSet::const_iterator it;
   for( it=m_setObserver.begin(); it!=m_setObserver.end(); it++ )
      (*it)->OnPreSendFile("STREAM", strRemoteFile, 0);

   const bool fRet=ExecuteDatachannelCommand(CDatachannelCmd(CDatachannelCmd::STOR()),
									strRemoteFile, CRepresentation(CType::Image()), fPasv, 0, pInput);

   for( it=m_setObserver.begin(); it!=m_setObserver.end(); it++ )
      (*it)->OnPostSendFile("STREAM", strRemoteFile, 0);
...
{
	if(RepresentationType(CRepresentation(CType::Image())) !=FTP_OK)
	 return FTP_ERROR;
   CReply Reply;
